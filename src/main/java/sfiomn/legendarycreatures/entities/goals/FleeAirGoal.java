...
public class FleeAirGoal<T extends LivingEntity> extends Goal {
    ...
    protected final float sprintDistance;
    ...
    public FleeAirGoal(PathfinderMob mob, Class<T> avoidClass, float maxDist, double walkSpeedModifier, double sprintSpeedModifier, float sprintDistance) {
        this(mob, avoidClass, (p_25052_) -> true, maxDist, walkSpeedModifier, sprintSpeedModifier, sprintDistance, EntitySelector.NO_CREATIVE_OR_SPECTATOR::test);
    }

    FleeAirGoal(PathfinderMob mob, @org.jetbrains.annotations.Nullable Class<T> avoidClass, Predicate<LivingEntity> avoidPredicate, float maxDist, double walkSpeedModifier, double sprintSpeedModifier, float sprintDistance, Predicate<LivingEntity> predicateOnAvoidEntity) {
        this.mob = mob;
        this.avoidClass = avoidClass;
        this.avoidPredicate = avoidPredicate;
        this.maxDist = maxDist;
        this.sprintDistance = sprintDistance;
        this.nextDetectionTick = mob.getRandom().nextInt(DETECTION_INTERVAL);
    ...
    public boolean canUse() {
        ...
        if (this.mob.tickCount < this.nextDetectionTick)
            return false;

        this.nextDetectionTick = this.mob.tickCount + DETECTION_INTERVAL;
    ...
    public void tick() {
        ...
        if (this.mob.distanceToSqr(this.toAvoid) < (double) (this.sprintDistance * this.sprintDistance)) {
