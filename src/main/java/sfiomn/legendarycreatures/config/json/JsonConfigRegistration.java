...
public class JsonConfigRegistration
{
	...
	private static void writeExampleJson(File jsonDir) {
		File exampleFile = new File(jsonDir, "_example-spawn.json");
		...
		try {
			String content = "{\n" +
					...
					"  }\n" +
					"}";
			FileUtils.write(exampleFile, content, (String) null);
			LegendaryCreatures.LOGGER.debug("Created _example-spawn.json");
		} catch (Exception e) {
			LegendaryCreatures.LOGGER.error("Error writing _example-spawn.json", e);
		}
	}
